 ...
 * 雷豆专区
 * */
public class KnowLeiZoneFragment extends BaseRefreshFragment<KnowZoneData.BeanBean> {

	private LinearLayoutManager manager = new LinearLayoutManager(getActivity());
	public static KnowLeiZoneFragment newInstance(int id) {

		Bundle args = new Bundle();
		args.putInt("sId", id);
		KnowLeiZoneFragment fragment = new KnowLeiZoneFragment();
	...
	@Override
	protected BaseRecyclerViewAdapter<KnowZoneData.BeanBean> getAdapter() {

//		LeiZoneAdapter leiZoneAdapter = new ;
//		leiZoneAdapter.notifyDataSetChanged();
		return new LeiZoneAdapter(getActivity(), null, manager);
	}
	...
	private void asyncSuccess(@NonNull KnowZoneData data, boolean isRefresh) {
		List<KnowZoneData.BeanBean> contentData = data.getBean();
